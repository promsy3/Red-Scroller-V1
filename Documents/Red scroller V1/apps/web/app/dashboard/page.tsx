...
export default async function Dashboard() {
  ...
  const firstName = user.email?.split('@')[0] ?? 'Counselor'
  ...
  return (
    <div className="rs-dashboard-page">
      ...
      <div className="rs-dashboard-header">
        <div>
          <h1 className="rs-dashboard-title">{greeting}, {firstName} 👋</h1>
            ...
            {recentActivity.map((entry: any) => (
              <div key={entry.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 8px', borderBottom: '1px solid #f3f4f6' }}>
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{entry.actor?.email || 'System'} · {new Date(entry.createdAt).toLocaleString()}</div>
                </div>
