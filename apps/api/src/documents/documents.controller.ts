...
export class DocumentsController {
  ...
  async registerDocument(
    ...
    const matter = await this.prisma.matter.findFirst({ 
      where: { id: body.matterId, firmId: user.firmId },
      include: { client: true }
    });
    if (!matter) throw new NotFoundException('Matter not found');
    ...
    // Notify all users with access to this matter about the new document
    const whereClause: any = {
      firmId: user.firmId,
      status: 'active',
      OR: [
        { role: 'admin' },
        { matterAccess: { some: { matterId: body.matterId } } }
      ]
    };
    
    if (matter.assignedTo) {
      whereClause.OR.push({ id: matter.assignedTo as string });
    }
    ...
    const uploader = await this.prisma.user.findUnique({ where: { id: req.user.sub } });
    const emailsToNotify = usersToNotify
      .map(u => u.email)
      .filter(email => email !== uploader?.email); // Don't notify the uploader

    if (emailsToNotify.length > 0) {
      await this.emailService.sendDocumentUploaded(
        emailsToNotify,
        body.name,
        matter.title,
        uploader?.name || uploader?.email || 'A team member'
      );
    }
